""" Runs MSA Transformer to create embeddings for each PDB ID in the input text file.
The embeddings are created in parallel in a specified number of batches (however, batch count be 1 on madoka).
...
def main(pdb_list_path, batches, msa_directory, output_directory):
    ...
    script_path = os.path.abspath(__file__).replace('run_all_batch_embeddings', 'batch_embeddings')
