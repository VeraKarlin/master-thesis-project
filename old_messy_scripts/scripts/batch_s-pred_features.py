...
""" Runs MSA Transformer to create s-pred features for each PDB ID in the input string.

Args:
    pdb_ids (str): PDB IDs as a string, seperated by a '+' sign.
    msa_directory (str): The directory where the MSA (.a3m) files are stored.
    output_directory (str): The directory where the s-pred feature (.pt) files should be created.
"""
...
def main(pdb_ids, msa_directory, output_directory):
    ...
    for pdb_id in pdb_ids.split('+'):
        ...
        try:
            ...
            torch.save(lstm_input, output_path)
            #print('SUCCESS: The embeddings for', pdb_id, 'were created.')
